...
<div class="content-header-left col-md-6 col-12 mb-2 breadcrumb-new">
    <h3 class="content-header-title mb-0 d-inline-block">Data Bidang Agent</h3>
...
<div class="content-header-right col-md-6 col-12">
    <div class="btn-group float-md-right">
        ...
        <button class="btn btn-info rounded-0 mb-1" id="createDataBidangAgentButton" type="button">Tambah</button>
    </div>
</div>
@endsection
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">List Data Bidang Agent</h4>
            ...
            <div class="card-content collapse show">
                <div class="card-body card-dashboard">
                    <table class="table table-striped table-bordered zero-configuration datatable">
                        ...
                        <tbody>
                            @php $no = 1; @endphp
                            @foreach($databidang_agent as $item)
                            <tr>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" style="min-width: 9rem !important">
                                            <button class="dropdown-item editDataBidangAgentButton" value="{{ $item->id }}">
                                                <i class="la la-edit"></i> Ubah
                                            </button>
                                            <button class="dropdown-item deleteDataBidangAgentButton" value="{{ $item->id }}">
...
<div class="modal fade" id="createDataBidangAgentModal">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header btn-info white">
                <h4 class="modal-title white">Tambah Data Bidang Agent</h4>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <form action="{{ route('databidang-agent.store') }}" method="post">
...
<div class="modal fade" id="editDataBidangAgentModal">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header btn-info white">
                <h4 class="modal-title white">Ubah Agent</h4>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <form action="" id="editDataBidangAgentForm" method="post">
                <div class="modal-body">
                    @csrf
                    @method("PUT")
                    <div class="form-group">
...
<div class="modal fade" id="deleteDataBidangAgentModal">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="" id="deleteDataBidangAgentForm" method="post">
                <div class="modal-footer">
                    @csrf
                    @method("DELETE")
                    <button type="button" class="btn grey btn-outline-secondary" data-dismiss="modal">Kembali</button>
...
<div class="modal fade" id="importExcel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form method="post" action="/databidang-agent-import" enctype="multipart/form-data">
            <div class="modal-content">
                <div class="modal-header bg-info white">
...
@section('script')
<script>
    $(document).on("click", "#createDataBidangAgentButton", function() {
        $("#createDataBidangAgentModal").modal();
    });

    $(document).on("click", ".editDataBidangAgentButton", function() {
        let id = $(this).val();
        $.ajax({
            method: "GET",
            url: "{{ route('databidang-agent.index') }}/" + id + "/edit"
            ...
            $("#editDataBidangAgentForm").attr("action", "{{ route('databidang-agent.index') }}/" + id)
            $("#editDataBidangAgentModal").modal();
        })
    });

    $(document).on("click", ".deleteDataBidangAgentButton", function() {
        let id = $(this).val();

        $("#deleteDataBidangAgentForm").attr("action", "{{ route('databidang-agent.index') }}/" + id)
        $("#deleteDataBidangAgentModal").modal();
    });
</script>
@endsection